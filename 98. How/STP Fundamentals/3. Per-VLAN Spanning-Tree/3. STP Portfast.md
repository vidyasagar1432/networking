...
**Do use PortFast on:**
- Access ports connected to end devices (PCs, printers, IP phones, cameras, servers).
- Ports where you expect devices to connect and disconnect frequently.
- Any port where connecting immediately is more important than loop protection (which already exists because it's an access port with no switch on the other end).

**Do NOT use PortFast on:**
- Trunk ports or ports connected to other switches.
- Ports where you could accidentally create a loop (typically ports you don't fully control).
- Any port where a BPDU could legitimately arrive from the connected device.

### PortFast and BPDU Guard

PortFast and BPDU Guard are often used together. While PortFast immediately transitions the port to Forwarding, BPDU Guard monitors the port for incoming BPDUs. If a BPDU is received on a PortFast-enabled port, BPDU Guard puts the port in errdisable state immediately.
...
This combination provides a strong security mechanism — a PC port comes up instantly, but if someone connects a switch to that port (potentially creating a loop), BPDU Guard immediately shuts the port down.

### PortFast and Voice VLAN

In networks with IP phones (voice VLAN), the phone sits between the PC and the switch. The switch port is configured with both a data VLAN and a voice VLAN. PortFast should still be enabled because this is still an access edge port.
...
The phone will use CDP or LLDP to discover the voice VLAN, and PortFast ensures the PC behind the phone gets immediate connectivity.
...
SW1# show running-config interface Ethernet0/3
Building configuration...
...
- **PortFast** allows a port to transition directly from Blocking to Forwarding, skipping Listening and Learning. It takes about 2 seconds instead of 30.
- Use PortFast on **access ports** connecting to end devices only, never on trunk ports or switch-to-switch links.
- **BPDU Guard** is commonly enabled alongside PortFast. It shuts down a PortFast port if a BPDU is received, preventing accidental loops.
- PortFast can be configured **per interface** (`spanning-tree portfast`) or **globally** on all access ports (`spanning-tree portfast default`).
- The **switchport host** command is a shortcut that configures PortFast, access mode, and BPDU Guard simultaneously.
- PortFast is safe on access ports because there is no switch on the other side, so loop formation is not a risk.
