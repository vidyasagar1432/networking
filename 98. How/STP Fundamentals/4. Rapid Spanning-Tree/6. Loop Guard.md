...
Loop Guard should be configured on **non-designated ports** (Root, Alternate, Backup) where a **unidirectional link failure** could create a loop. The most common use cases:

**Access layer switches with redundant uplinks:**
- Configure Loop Guard on the Alternate port of the access switch. If the primary uplink fails and BPDUs stop arriving on the Alternate port, Loop Guard prevents the port from transitioning to Designated and creating a loop.

**Distribution layer:**
- Configure Loop Guard on all non-designated ports facing access switches. This protects against scenarios where an access switch stops receiving BPDUs due to a unidirectional link failure.

**Where NOT to use Loop Guard:**
- On **Root Ports** — A Root Port that stops receiving BPDUs will attempt to become the new Root Bridge. Loop Guard would prevent this legitimate failover, potentially isolating the switch.
- On ports connected to **end devices** (these don't send BPDUs, so the port would be stuck in Loop Inconsistent permanently).
- On ports where you have **PortFast** enabled (these are edge ports that don't expect BPDUs at all).

### Loop Guard vs UDLD

| Feature | Loop Guard | UDLD |
|---------|------------|------|
| Purpose | Prevents loops from BPDU loss | Detects unidirectional links |
| Mechanism | Monitors BPDU reception | Sends UDLD probes on the link |
| Failure type | BPDU loss (any cause) | Physical unidirectional link |
| Action | Port goes to Loop Inconsistent | Port goes to errdisable |
| Recovery | Automatic when BPDUs resume | Manual or errdisable recovery |
| Configuration | Per port (`spanning-tree guard loop`) | Per port (`udld port aggressive`) |
| Protection scope | STP only | Layer 1 and Layer 2 |

### Loop Guard configuration guidelines

```plaintext
! Enable Loop Guard on a specific interface
interface Ethernet0/2
 spanning-tree guard loop

! Enable Loop Guard globally (all non-designated ports)
...
- Loop Guard performs **no action** on ports that are already Designated. It only monitors non-designated (Root, Alternate, Backup) ports.
- If a port is configured with both Root Guard and Loop Guard, **Root Guard takes precedence**.
- Loop Guard and BPDU Guard should not be configured on the same port (their purposes conflict).
- Loop Guard is recommend on **all non-designated ports** in a network for maximum resilience.

### Common CCNA exam questions about Loop Guard

1. **What problem does Loop Guard solve?**
   Unidirectional link failures or software issues that cause a switch to stop receiving BPDUs but the port remains up/up.

2. **What state does the port enter?**
   **Loop Inconsistent** — shown as `BKN*` with `LOOP_Inc` in `show spanning-tree`.

3. **How does the port recover?**
   **Automatically** — once the port starts receiving BPDUs again, Loop Guard unblocks it and it returns to its original role (Alternate or Root).

4. **What happens if Loop Guard is configured on a port connected to a host?**
   The port will go to Loop Inconsistent because the host never sends BPDUs. Loop Guard should not be used on edge ports.

## Summary

- **Loop Guard** prevents a non-designated port (Root, Alternate, Backup) from transitioning to Designated when it stops receiving BPDUs.
- The port enters **Loop Inconsistent** state (`BKN* LOOP_Inc`) until BPDUs resume.
- It protects against **unidirectional link failures** and software bugs that cause BPDU loss.
- It is configured per-port or globally and recovers automatically.
- Do not use Loop Guard on Root Ports, edge ports, or ports without BPDU expectations.
- Use `show spanning-tree inconsistentports` to find all ports in inconsistent state.
