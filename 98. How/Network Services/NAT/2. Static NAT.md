...
Static address translation can also be configured in the opposite direction for the host on the outside. For example, the Outside host with Outside Global address 8.8.8.8 could be represented with an Outside Local address 10.1.1.8 on the inside, depending on the network requirements. Let's define these remaining two terms to complete the picture:

-   The term **"Outside Global"** refers to the real IP address of the outside host from the perspective of the outside network — its actual address on the Internet. In our example, the Google DNS server at 8.8.8.8 is reachable via that exact address globally. This is the address assigned to the outside device.

-   The term **"Outside Local"** refers to the address of the outside host as seen from the inside network. In a basic Static NAT configuration, the Outside Local and Outside Global addresses are typically the same because we only translate the inside addresses. However, as mentioned earlier, you could configure a static mapping in the opposite direction so that the outside host 8.8.8.8 appears as 10.1.1.8 to devices on the inside.
...
Let's trace a packet from PC1 (10.1.1.1) to the Google DNS server (8.8.8.8) to see how Static NAT handles translation in both directions. Unlike Dynamic NAT, where mappings are created on the fly, the static mapping is pre-populated in the NAT table as soon as the configuration is applied. You can see it immediately with the `show ip nat translations` command, even before any traffic flows.
...
2. The **NAT router** receives the packet on its inside interface (Ethernet0/0), which has `ip nat inside` configured. The router checks its NAT table.
3. The router finds the static entry `--- 37.3.1.1 10.1.1.1 --- ---` and translates the source IP from **10.1.1.1** to **37.3.1.1**. The destination is left unchanged.
...
5. The **Google DNS server** receives the packet and sees the source IP as **37.3.1.1**. It sends its reply back to that address.
...
2. The **NAT router** receives the packet on its outside interface (Ethernet0/1), which has `ip nat outside` configured. The router checks its NAT table.
3. The router finds the same static entry and translates the destination IP from **37.3.1.1** back to **10.1.1.1**.
...
This symmetric translation — source IP translated on the way out, destination IP translated on the way back — is what makes Static NAT work seamlessly for bidirectional communication. You can also use `show ip nat statistics` to see how many translations are active and the total number of hits on each translation entry, which is useful for verifying that traffic is being translated as expected.

### Summary

Static NAT is a simple, predictable one-to-one mapping between an inside local IP address and an inside global IP address. Here are the key takeaways:

-   **One-to-one mapping** — Each inside local address is permanently mapped to a specific inside global address.
-   **No address conservation** — Static NAT consumes one public IP address for each internal host. It does not help conserve the IPv4 address space.
...
-   **Bidirectional** — Static NAT allows hosts on the outside to initiate connections to the inside because the mapping is always present in the NAT table (it does not need to be created dynamically).
