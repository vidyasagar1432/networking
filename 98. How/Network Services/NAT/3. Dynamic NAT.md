...
Let's trace a packet from PC1 (10.1.1.1) to the Google DNS server (8.8.8.8) to see how Dynamic NAT translates the packets compared to Static NAT.
...
4. The router picks the first available address from the pool (e.g., **37.3.1.1**) and creates a dynamic entry in the NAT table: (10.1.1.1 → 37.3.1.1).
...
If a fifth host (say, PC5 at 10.1.1.5) tries to send traffic when all 20 pool addresses are already allocated, the router discards the packet and increments the **Misses** counter in the NAT statistics. The host will not be able to reach the Internet until an existing mapping times out (after 24 hours of inactivity) and a public IP is returned to the pool.
...
- **One-to-one while active**: Even though the mapping is dynamic, each active translation still consumes one public IP address — it is not yet multiplexing.
...
- **Monitor Misses**: The `sh ip nat statistics` command's Misses counter is critical — a non-zero value means hosts are being dropped because the pool is exhausted.
