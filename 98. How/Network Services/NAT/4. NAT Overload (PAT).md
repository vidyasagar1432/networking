...
6. The router dynamically creates NAT table entries mapping each translated socket — unlike Static NAT, these entries are not pre-populated. An inactivity timer starts for each entry (typically 60 seconds for UDP, 24 hours for TCP).
7. The router forwards the packets out the outside interface.
8. The **server** sees three TCP sessions all coming from the same host **37.3.1.1** but with different source ports. It treats them as independent sessions and sends replies back to each respective socket.
...
- **Port uniqueness** — The router assigns a unique source port to each translation to differentiate between simultaneous sessions.
...
- **Timeouts** — PAT entries have shorter timeouts than Dynamic NAT: typically 60 seconds for UDP and 24 hours for TCP.
...
In the next lessons, we will practice these concepts with labs and tests.