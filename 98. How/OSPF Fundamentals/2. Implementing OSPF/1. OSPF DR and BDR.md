...
**What happens next?** Even though R1 has a higher priority (50) than the current DR and BDR, it will NOT replace them. Once a DR and BDR are elected, they remain in their roles until they fail or their OSPF process is reset. This is by design — OSPF avoids unnecessary DR/BDR changes that would cause disruption.

R1 simply becomes a DROTHER (a non-DR/BDR router) and forms full adjacencies with the DR and BDR only. It does not form full adjacencies with other DROTHERs.

## Configuring DR/BDR

In most networks, you don't need to explicitly configure DR/BDR — the election happens automatically based on the highest OSPF priority and Router ID. However, you may want to influence the election to ensure that a more capable router becomes the DR.
...
The priority can be set from 0 to 255. A priority of 0 means the router will never become DR or BDR. By default, the priority is 1 on Cisco routers.

**Key rules for DR/BDR election:**

1. The router with the **highest OSPF priority** becomes the DR. If there is a tie, the router with the highest Router ID wins.
2. The router with the **second-highest priority** becomes the BDR.
3. **Priority is a per-interface setting**, not a global OSPF setting.
4. A priority of **0** means the router is **ineligible** for DR/BDR election. It will always be a DROTHER.
5. Once elected, the DR and BDR **do not change** unless they fail — even if a higher-priority router joins later.

## Verifying DR and BDR

Use the `show ip ospf interface` command to see the DR/BDR status.
...
GigabitEthernet0/0 is up, line protocol is up
  ...
  Neighbor Count is 3, Adjacent neighbor count is 2
    ...
    Adjacent with neighbor 10.0.0.3  (DROTHER)
    Adjacent with neighbor 10.0.0.4  (DROTHER)
```

Key information from this output:
- **State DR** — This router (10.0.0.1) is the DR.
- **Designated Router** — Shows the DR's Router ID and interface IP.
- **Backup Designated Router** — Shows the BDR's Router ID and interface IP.
- **Adjacent neighbor count** — Shows 2, even though the neighbor count is 3. This is because the DR only forms full adjacencies with the BDR and DROTHERs, but not with every neighbor.
- **DROTHER** — Neighbors that are neither DR nor BDR.

## DR/BDR on Different Network Types

DR/BDR election only happens on **broadcast** and **non-broadcast multiaccess (NBMA)** network types. On point-to-point links (like serial connections), there is no DR/BDR election because only two routers are connected.

| Network Type | DR/BDR Election | Default |
|--------------|-----------------|---------|
| Broadcast (Ethernet) | Yes | Default on Ethernet interfaces |
| Point-to-Point | No | Default on serial interfaces |
| Non-Broadcast (NBMA) | Yes | Rarely used |
| Point-to-Multipoint | No | Rarely used |

## Summary

- **DR (Designated Router)** and **BDR (Backup Designated Router)** reduce the number of adjacencies and LSA flooding on multiaccess networks.
- All routers form full adjacencies with the DR and BDR only. DROTHERs form only 2-Way state between each other.
- The DR is elected based on the **highest OSPF priority** (default is 1), with the **highest Router ID** as the tiebreaker.
- **A DR once elected stays in its role** — a higher-priority router joining later does not preempt the existing DR.
- Use `ip ospf priority 0-255` on the interface to influence the election. Setting priority to 0 makes the router ineligible.
- DR/BDR election only occurs on **broadcast** and **NBMA** network types. Point-to-point links don't use DR/BDR.
