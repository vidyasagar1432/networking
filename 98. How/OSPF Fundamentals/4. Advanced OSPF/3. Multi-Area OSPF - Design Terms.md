...
Based on their position in the network, OSPF routers are classified into four types.

**Internal Router** — All interfaces are in the same OSPF area. Internal routers have a complete LSDB for their area and run the SPF algorithm to calculate routes within that area.

**Area Border Router (ABR)** — Interfaces in multiple areas, including at least one in Area 0. ABRs generate Type 3 LSAs to advertise networks between areas. They maintain a separate LSDB for each connected area.

**Backbone Router** — Has at least one interface in Area 0. ABRs and any routers with interfaces only in Area 0 are backbone routers.

**Autonomous System Boundary Router (ASBR)** — Redistributes routes from other routing protocols (RIP, EIGRP, BGP, static) into OSPF. An ASBR can be in any area.

## OSPF Area Types

OSPF defines several area types to control the types of LSAs allowed within them.

**Standard Area** — The default area type. All LSA types (1, 2, 3, 4, 5) are allowed. This is the most common area type.

**Backbone Area (Area 0)** — The central area that connects all other areas. All traffic between different areas must pass through Area 0.

**Stub Area** — Blocks Type 5 LSAs (external routes). The ABR injects a default route (Type 3 LSA with 0.0.0.0/0) into the stub area. This reduces the LSDB size.
- Type 3 LSAs are allowed (inter-area routes).
- Type 4 and 5 LSAs are blocked.
- No ASBRs allowed in stub areas.

**Totally Stubby Area** — Cisco proprietary. Blocks Type 3, 4, and 5 LSAs. Only Type 1 and 2 LSAs plus a default route from the ABR.
- Even smaller LSDB than stub area.
- All inter-area traffic goes through the ABR's default route.

**Not-So-Stubby Area (NSSA)** — Similar to stub area but allows an ASBR. External routes from the ASBR are carried as Type 7 LSAs, which the ABR converts to Type 5.
- Type 5 LSAs are blocked.
- Type 7 LSAs are used for external routes within the NSSA.

### Area type comparison

| Area Type | Type 1/2 | Type 3 | Type 4/5 | Type 7 | Default Route |
|-----------|----------|--------|----------|--------|---------------|
| Standard | Yes | Yes | Yes | No | No (unless configured) |
| Backbone | Yes | Yes | Yes | No | No |
| Stub | Yes | Yes | No | No | Yes (injected by ABR) |
| Totally Stubby | Yes | No | No | No | Yes (injected by ABR) |
| NSSA | Yes | Yes | No | Yes | Optional |

## Design Rules and Best Practices

- **Area 0 must be contiguous** — All non-backbone areas must connect directly to Area 0. If they don't, you must use virtual links (CCNP topic).
- **All traffic between areas flows through Area 0** — There is no direct routing between two non-backbone areas.
- **Keep areas small** — For optimal SPF performance, keep each area to a few hundred routers maximum.
- **Use stub areas where possible** — If an area has no ASBR and doesn't need external routes, make it a stub or totally stubby area to reduce LSDB size.
- **Summarize routes at ABRs** — Use `area range` on ABRs to summarize networks, reducing the number of Type 3 LSAs.
- **Place the most stable and powerful routers in Area 0** — Area 0 carries the most routing information and should have the most reliable equipment.

## Summary

- Multi-area OSPF improves scalability by reducing LSDB size and limiting the impact of topology changes.
- **Router types:** Internal, ABR, Backbone, ASBR.
- **Area types:** Standard, Backbone, Stub, Totally Stubby, NSSA.
- **Area 0 must be contiguous** and all other areas must connect to it.
- Stub and totally stubby areas reduce LSDB size at the cost of routing flexibility.
