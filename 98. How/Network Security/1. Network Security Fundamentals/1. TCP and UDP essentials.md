...
Let's look at some of TCP's key features that make it reliable.

### Sequence and Acknowledgment Numbers

When TCP sends data, it assigns a **sequence number** to each byte. The receiver uses these numbers to reassemble data in the correct order and to detect missing segments.

When the receiver gets data, it sends back an **acknowledgment number** equal to the next expected byte. This tells the sender "I received everything up to this point."

For example:
- Host A sends bytes 1-100 with sequence number 1.
- Host B receives them and sends ACK = 101 ("I'm expecting byte 101 next").
- Host A sends bytes 101-200.
- This continues until all data is transferred.

If Host A sends bytes 1-100, 101-200, and 201-300, but the second segment (101-200) is lost, Host B will keep sending ACK = 101 (indicating it's still waiting for byte 101). When Host A receives three duplicate ACKs for the same sequence number, it retransmits the missing segment immediately. This is called **fast retransmit**.

### TCP Window and Flow Control

TCP uses a **sliding window** mechanism to control how much data can be sent before waiting for an acknowledgment. The window size is advertised by the receiver in the TCP header to indicate how much buffer space it has available.

If the receiver is slow and its buffer is filling up, it advertises a smaller window. The sender then reduces its transmission rate. This prevents a fast sender from overwhelming a slow receiver. This is called **flow control**.

The initial window size is set during the three-way handshake. As data flows, the window size can change dynamically based on the receiver's available buffer space.

### TCP Congestion Control

TCP also handles congestion on the network itself. It uses several algorithms to adjust the transmission rate when it detects packet loss.

**Slow Start** — TCP starts by sending a small number of segments (typically one). For each ACK received, it doubles the sending rate. This exponential growth continues until it reaches a threshold or experiences packet loss.

**Congestion Avoidance** — After reaching the threshold, TCP switches to a more conservative linear increase, adding one segment per round-trip time.

**Fast Recovery** — When a packet is lost, TCP reduces its transmission rate and then gradually increases again.

These mechanisms ensure that TCP doesn't make network congestion worse by flooding the network with retransmissions.

## Port Number Ranges

Both TCP and UDP use port numbers to identify applications. Ports are divided into three ranges:

**Well-known ports (0-1023)** — Reserved for system services. Examples:
  - 20/21: FTP
  - 22: SSH
  - 23: Telnet
  - 25: SMTP
...
**Registered ports (1024-49151)** — Assigned to specific applications by IANA. Examples:
  - 1433: Microsoft SQL Server
  - 3306: MySQL
  - 3389: RDP
  - 8080: HTTP alternate

**Dynamic/private ports (49152-65535)** — Used temporarily by client applications when connecting to a server. The client's OS assigns a random port from this range for each outgoing connection.
...
| Use cases | Web, email, file transfer, remote access | VoIP, DNS, video streaming, online gaming |

## Security Implications

For network security professionals, understanding TCP and UDP is critical.

**TCP SYN Flood** — An attacker sends many TCP SYN requests without completing the handshake, exhausting the server's resources. This is a type of denial-of-service (DoS) attack.

**Port Scanning** — Attackers probe which TCP and UDP ports are open on a target to identify vulnerable services. Common techniques include SYN scan, Connect scan, and UDP scan.

**TCP Session Hijacking** — If an attacker can predict or capture TCP sequence numbers, they can inject malicious data into an established TCP session.

**UDP Amplification** — Attackers can spoof a victim's IP address and send small queries to UDP services (like DNS or NTP) that return large responses. The responses flood the victim. This is a common DDoS technique.

Understanding which protocols use TCP vs UDP helps you configure firewall rules correctly. For example, DNS needs UDP 53 open, but web traffic needs TCP 80/443.

## Summary

- **TCP** is connection-oriented, reliable, and ensures in-order delivery. It uses sequence numbers, acknowledgments, and windowing to provide flow control and congestion control.
- **UDP** is connectionless and lightweight. It provides no reliability guarantees but has minimal overhead.
- Both protocols use **port numbers** (0-65535) to identify applications, divided into well-known, registered, and dynamic ranges.
- Understanding TCP and UDP is essential for **network security** — to recognize attacks like SYN floods, port scans, and UDP amplification.
