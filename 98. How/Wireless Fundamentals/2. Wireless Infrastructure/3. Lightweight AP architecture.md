...
When a lightweight AP receives a data frame from a wireless client, the following happens:

1. The LAP encapsulates the entire 802.11 frame inside a CAPWAP data packet.
2. The CAPWAP packet is sent over UDP port 5247 to the WLC through the CAPWAP data tunnel.
3. The WLC receives the packet, decapsulates the 802.11 frame, and converts it to an 802.3 Ethernet frame.
4. The WLC bridges the frame to the appropriate wired VLAN based on the SSID-to-VLAN mapping.
5. For return traffic, the WLC receives the Ethernet frame, encapsulates it in CAPWAP, and sends it to the LAP, which then transmits it wirelessly.

This centralized forwarding model is called **Centralized (or Tunneled) Mode**.

### FlexConnect (Local Switching)

Cisco also offers **FlexConnect** mode (formerly known as Hybrid Remote Edge Access Point or H-REAP). In FlexConnect mode, the lightweight AP can locally switch client traffic at the AP itself instead of tunneling everything to the WLC.

**FlexConnect advantages:**
- Traffic does not need to traverse the WAN to reach the WLC if the AP is at a remote branch.
- Reduces latency for local traffic (Internet-bound or local LAN traffic).
- Provides survivability — if the WAN link to the WLC goes down, the AP continues to serve clients.

**FlexConnect modes:**
- **Centralized Switching:** All traffic goes to the WLC (default).
- **Local Switching:** Client data is bridged locally at the AP. Only control traffic goes to the WLC.
- **FlexConnect with ACL:** Local switching with ACLs applied at the AP for local traffic.

### WLC Deployment Models

| Model | Description | Use Case |
|-------|-------------|----------|
| **Unified (Cisco 9800 series)** | Physical appliance controller | Large campus deployments |
| **Embedded (in switch)** | WLC module inside a Catalyst switch | Medium-sized deployments |
| **Cloud-based (Meraki)** | Controller as a cloud service | Distributed branch offices |
| **Mobility Express** | Controller functionality embedded in the AP | Small/medium businesses |
| **vWLC (virtual)** | Controller running as a VM | Data center or lab environments |

### Comparing Autonomous vs Lightweight Architectures

| Feature | Autonomous AP | Lightweight AP (WLC) |
|---------|---------------|---------------------|
| Management | Per-AP (manual) | Centralized (WLC) |
| Configuration | Local CLI/GUI | WLC web interface / templates |
| RF management | Manual | RRM (automatic) |
| Security policies | Per-AP | Centralized |
| Roaming | Layer 2 only | Layer 2 and Layer 3 |
| Firmware updates | Per-AP manual | Push from WLC |
| Traffic forwarding | Local bridging | CAPWAP tunnel to WLC (or FlexConnect) |
| Scalability | Limited | High |
| Cost | Lower (no controller) | Higher (controller + APs) |

### Advantages of Lightweight AP Architecture

- **Centralized management** — One WLC manages hundreds of APs. Configuration changes are pushed to all APs automatically.
- **Automatic RF optimization** — Radio Resource Management (RRM) dynamically adjusts channels, power levels, and coverage.
- **Seamless roaming** — Clients can roam across APs while staying on the same subnet, even across L3 boundaries (using mobility tunneling).
- **Improved security** — Rogue AP detection, client authentication centralized, and encryption keys managed by the WLC.
- **Faster firmware upgrades** — Upload the firmware to the WLC once; all APs are updated automatically.
- **Fault tolerance** — APs can fail over to backup WLCs if the primary WLC goes down.

### Summary

- **Lightweight AP (LAP)** architecture separates the control plane (WLC) from the data plane (AP), known as **split-MAC**.
- LAPs use **CAPWAP tunnels** (control on UDP 5246, data on UDP 5247) to communicate with the WLC.
- By default, all client traffic is tunneled to the WLC for centralized forwarding (**Centralized Mode**).
- **FlexConnect** mode allows local switching at the AP for remote/branch locations.
- **WLC deployment models** include physical appliances, embedded modules, cloud-based (Meraki), Mobility Express, and virtual WLCs.
- LAP architecture is the standard for enterprise wireless deployments due to its scalability, centralized management, and advanced features like RRM and seamless roaming.
