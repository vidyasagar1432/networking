...
**1. Proper channel planning:**
- Use non-overlapping channels (1, 6, 11 in 2.4 GHz).
- In 5 GHz, use the widest channels (40/80 MHz) only when signal conditions are excellent.
- Avoid using the same channel on APs that are within range of each other.

**2. Adjust transmit power:**
- Lower transmit power reduces co-channel interference by shrinking cell sizes.
- APs that are too "hot" cause more interference to neighboring APs.
- Modern WLCs with RRM (Radio Resource Management) adjust power and channels automatically.

**3. Physical separation:**
- Place APs so that walls and floors provide natural signal attenuation between same-channel APs.
- At least 2 walls between same-channel APs is a good rule of thumb.

**4. Use the 5 GHz band when possible:**
- 5 GHz has many more non-overlapping channels than 2.4 GHz.
- 5 GHz signals attenuate faster, which naturally reduces interference range.
- 2.4 GHz is crowded (Wi-Fi + Bluetooth + microwaves + cordless phones).

**5. Conduct a site survey:**
- A proper site survey measures RSSI, SNR, and noise floor across the coverage area.
- It identifies dead zones, co-channel interference, and non-Wi-Fi interference sources.

### Common CCNA exam questions about Interference, RSSI, and SNR

1. **What is the difference between RSSI and SNR?**
   - RSSI = Received Signal Strength Indicator, measured in dBm. It is the absolute power level of the received signal.
   - SNR = Signal-to-Noise Ratio, measured in dB. It is the difference between the RSSI and the noise floor.

2. **What is a good SNR value for Wi-Fi?**
   - Above 20 dB is excellent, 10-20 dB is moderate, below 10 dB is poor.

3. **Why does 5 GHz have shorter range than 2.4 GHz?**
   - Because FSPL increases with frequency. Higher frequencies (5 GHz) experience more path loss over the same distance.

4. **What channels should you use in 2.4 GHz to avoid neighboring channel interference?**
   - Channels 1, 6, and 11 are non-overlapping in 2.4 GHz.

5. **What is the difference between Co-Channel Interference (CCI) and Neighboring Channel Interference (NCI)?**
   - CCI = Same channel, overlapping coverage. NCI = Adjacent channels with overlapping spectrum.

6. **What is noise floor?**
   - The background level of unwanted RF signals in the environment. It sets the minimum signal level that a receiver can detect.

### Summary

- **RSSI** measures the received signal power in dBm. Typical values range from -30 dBm (excellent) to -90 dBm (very weak).
- **SNR** is the difference between RSSI and the noise floor. Higher SNR = better signal quality.
- **Co-Channel Interference (CCI)** occurs when multiple devices use the same channel. Mitigated by proper cell sizing and channel reuse.
- **Neighboring Channel Interference (NCI)** occurs when overlapping channels interfere. Avoided by using non-overlapping channels (1, 6, 11 in 2.4 GHz).
- **Non-WiFi interference** comes from microwaves, Bluetooth, cordless phones, etc. May require switching bands.
- **Free Space Path Loss (FSPL)** is signal loss over distance. It increases with both distance and frequency.
- **Dynamic Rate Shifting** allows Wi-Fi to adapt modulation based on signal conditions.
- The 2.4 GHz band has longer range but fewer channels and more interference. The 5 GHz band has shorter range but more channels and less interference.
